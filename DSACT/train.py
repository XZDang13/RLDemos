...
class Trainer:
    def __init__(self, env_name:str, env_num:int, seed:int=0, n_step:int=1):
        ...
        self.global_step = 0
        WandbLogger.init_project("RLDemos", f"DSACT-{env_name}-{seed}")
    ...
    def update(self, num_iteration:int, batch_size:int):
        ...
        for _ in range(num_iteration):
            ...
            critic_loss_dict = DSACT.compute_critic_loss(self.actor, self.critic, self.critic_target, 
                                                    obs_batch, action_batch, reward_batch, next_obs_batch, done_batch, self.alpha,
                                                    self.gamma, self.tau_b, discount=discount_batch)
            
            critic_loss = critic_loss_dict["loss"]
            q1 = critic_loss_dict["q1"]
            q2 = critic_loss_dict["q2"]
            q1_target = critic_loss_dict["q1_target"]
            q2_target = critic_loss_dict["q2_target"]
            ...
            policy_loss_dict = DSACT.compute_policy_loss(self.actor, self.critic, obs_batch.detach(), self.alpha, self.regularization_weight)
            ...
            q1_target_buffer.append(q1_target.item())
            q2_target_buffer.append(q2_target.item())
        ...
        avg_q1_target = np.mean(q1_target_buffer)
        avg_q2_target = np.mean(q2_target_buffer)
        ...
        train_info = {
            ...
            "update/avg_q1_target": avg_q1_target,
            "update/avg_q2_target": avg_q2_target,
        }
...
if __name__ == "__main__":
    trainer = Trainer("HalfCheetah-v5", 20, seed=50, n_step=1)
