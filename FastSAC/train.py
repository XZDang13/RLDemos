...
from RLAlg.alg.fast_sac import FastSAC
...
class Trainer:
    def __init__(self, env_name: str, env_num: int, seed: int = 0, n_step: int = 1):
        ...
        self.envs = gymnasium.vector.SyncVectorEnv(
            [lambda: self.setup_env(env_name) for _ in range(env_num)]
        )
        ...
        if not isinstance(self.envs.single_action_space, gymnasium.spaces.Box):
            raise ValueError("FastSAC demo only supports continuous Box action spaces.")
        ...
        self.global_step = 0
        WandbLogger.init_project("RLDemos", f"FastSAC-{env_name}-{seed}")
    ...
    def update(self, num_iteration: int, batch_size: int):
        ...
        for _ in range(num_iteration):
            batch = self.replay_buffer.sample_n_step_batch(
                self.batch_keys,
                batch_size,
                **self.sample_batch_kwargs,
            )
            ...
            critic_loss_dict = FastSAC.compute_critic_loss(
                self.actor,
                self.critic,
                self.critic_target,
                obs_batch,
            ...
            policy_loss_dict = FastSAC.compute_policy_loss(
                self.actor,
                self.critic,
                obs_batch.detach(),
                alpha,
            ...
            alpha_loss_dict = FastSAC.compute_alpha_loss(
                self.actor,
                self.log_alpha,
                obs_batch.detach(),
                self.target_entropy,
            ...
            FastSAC.update_target_param(self.critic, self.critic_target, self.tau)
...
if __name__ == "__main__":
    trainer = Trainer("HalfCheetah-v5", 20, seed=100, n_step=1)

    trainer.train(total_frames=1_000_000, num_iteration=25, batch_size=500)
